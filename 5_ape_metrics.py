...
all_shares.extend([y["value"] / 175.0 for y in purchases])
...
with open("results.tsv", "w+") as f:
    f.write("value\ttype\turls\n")
    for record in portfolios:
        f.write(f'{str(record["value"])}\t{record["type"]}\t{record["urls"]}\n')
    for record in purchases:
        f.write(f'{str(record["value"])}\t{record["type"]}\t{record["urls"]}\n')
...
pur_values = [x["value"] / 175.0 for x in purchases]
