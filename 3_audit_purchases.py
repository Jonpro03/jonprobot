...
purchase_table = mongo["computershare_posts"]['purchases']
gme = yf.Ticker("GME")
close_val = gme.info["regularMarketPreviousClose"]


def get_reddit_post(post_id):
    record = purchase_table.find_one({"_id": post['id']})
    if record:
        return record
...
def delete_post(post):
    ...
    try:
        ...
        pass
    purchase_table.update_one({"_id": post['id']}, {"$set": post})
...
def audit_cv_failures():
    # select * from purchase_table where value = nul and duped_by = [] and audited = false
    posts = list(purchase_table.find({"$and": [
        {"audited": False},
        {"deleted": False},
        {"value": None},
        {"duped_by": []}
...
def manual_audit():
    ...
    while True:
        ...
        if post is not None:
            ...
            purchase_table.insert_one(post)
            get_new_value(post)
...
def identify_dupes():
    ...
    for post in posts:
        ...
        dupes = list(purchase_table.find({"img_hash": post['img_hash']}))
        if len(dupes) > 1:
            ...
            for dupe in dupes:
                if post["id"] != dupe["id"]:
                    ...
                    dupe["value"] = 0
                    purchase_table.update_one({"_id": dupe['id']}, {"$set": dupe})
